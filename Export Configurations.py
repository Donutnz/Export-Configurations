...
def modifySTEP(fileName, app, partName):
	try:
...
def run(context):
	...
	try:
		...
		app.log("Starting Configs Exporter...")
		...
		if design.isConfiguredDesign:
			...
			app.log("Exporting Configured Design Rows: {}".format(topTable.rows.count))
			...
			for row in topTable.rows:
				...
				modifySTEP(fileName, app, row.name)
				
				app.log("Done: {}\n".format(row.name))
				filesCnt+=1

		else:
			app.log("Exporting Root Component")
			fileName=targetFolderPath+'/'+design.rootComponent.partNumber+".step"
			...
			#Reopen exported STEP file to tweak names and add header stuff.
			modifySTEP(fileName, app, design.rootComponent.partNumber)
			filesCnt+=1

		ui.messageBox("Operation Complete. "+str(filesCnt)+" Step File/s Exported to: "+targetFolderPath+".")
		app.log("Done")
